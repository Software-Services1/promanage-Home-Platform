...
<aside x-bind:class="nav ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'"
  ...
  <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
    <?php $__currentLoopData = $nav; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php $visible = isset($n['can']) ? auth()->user()->can($n['can']) : auth()->user()->hasAnyRole($n['roles']); ?>
      <?php if($visible): ?>
        <?php $active = str_starts_with($current, explode('.', $n['route'])[0]); ?>
        <a href="<?php echo e(route($n['route'])); ?>" x-on:click="nav=false"
           class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition hover:bg-white/5 <?php echo e($active ? 'active' : ''); ?>">
          <span class="ic w-8 h-8 rounded-lg grid place-items-center bg-white/5 <?php echo e($active ? '' : 'text-white/60'); ?>">
            <svg viewBox="0 0 24 24" class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"><path d="<?php echo e($n['icon']); ?>"/></svg>
          </span>
          <span><?php echo e($n['label']); ?></span>
        </a>
      <?php endif; ?>
  ...
  <div class="p-3 border-t border-white/10">
    <div class="flex items-center gap-3 px-2 py-2 rounded-xl">
      <div class="w-9 h-9 rounded-full grid place-items-center text-white font-bold ff-display" style="background:<?php echo e($avatarColor(auth()->id())); ?>"><?php echo e(mb_substr(auth()->user()->name,0,1)); ?></div>
      <div class="min-w-0 flex-1">
        <div class="text-white text-sm font-semibold truncate"><?php echo e(auth()->user()->name); ?></div>
        <div class="text-[11px] text-white/45"><?php echo e($roleLabels[auth()->user()->primaryRole()] ?? ''); ?></div>
      </div>
...
<?php /**PATH D:\markting\promanage\resources\views/partials/sidebar.blade.php ENDPATH**/ ?>