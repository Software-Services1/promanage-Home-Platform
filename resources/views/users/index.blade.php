...
@section('content')
@php $roleOptions = ['admin'=>'مشرف عام','supervisor'=>'مشرف المحتوى','designer'=>'مصمم جرافيك','editor'=>'مونتير ومصمم']; @endphp
<div x-data="{ open:false, mode:'create', form:{} }">
  <div class="flex items-center justify-between mb-4">
    <div class="text-sm text-muted">{{ $users->count() }} موظفين</div>
        ...
        <tr class="border-b border-line/60 hover:bg-canvas/60">
          <td class="p-4">
            <div class="flex items-center gap-3">
              ...
              <div><div class="font-semibold">{{ $u->name }}</div><div class="text-[11px] text-muted">{{ $u->email }}</div></div>
            </div>
          </td>
          <td class="p-4"><span class="text-xs font-semibold bg-violet-50 text-brand px-2 py-1 rounded-lg">{{ $roleOptions[$u->primaryRole()] ?? '—' }}</span></td>
          <td class="p-4 tnum text-muted">{{ $u->target ?: '—' }}</td>
          <td class="p-4 tnum text-muted">{{ optional($u->join_date)->format('Y-m-d') ?? '—' }}</td>
          <td class="p-4">
            @if($u->is_active)<span class="text-emerald-700 text-xs font-semibold">نشط</span>
            @else<span class="text-rose-500 text-xs font-semibold">معطّل</span>@endif
          </td>
          <td class="p-4">
            <div class="flex items-center gap-1 justify-end">
              <button x-on:click="mode='edit'; form={ id:{{ $u->id }}, name:@js($u->name), email:@js($u->email), role:@js($u->primaryRole()), salary:{{ (float)$u->salary }}, target:{{ (int)$u->target }}, supervisor_share:{{ (float)$u->supervisor_share }}, join_date:@js(optional($u->join_date)->format('Y-m-d')) }; open=true"
              ...
              <form method="POST" action="{{ route('users.reset',$u) }}">@csrf
              ...
              <form method="POST" action="{{ route('users.toggle',$u) }}">@csrf
                <button class="w-8 h-8 rounded-lg grid place-items-center text-muted hover:text-rose-600 hover:bg-rose-50" title="{{ $u->is_active ? 'تعطيل':'تفعيل' }}">
              ...
              @if($u->id !== auth()->id())
                <form method="POST" action="{{ route('users.destroy',$u) }}" onsubmit="return confirm('حذف المستخدم {{ $u->name }} نهائياً؟')">@csrf @method('DELETE')
  ...
  <div x-cloak x-show="open" class="fixed inset-0 z-50 grid place-items-center p-4">
    ...
    <form method="POST" x-bind:action="mode==='create' ? '{{ route('users.store') }}' : '/users/'+form.id"
      ...
      <div class="grid grid-cols-2 gap-3">
        <div><label class="text-xs font-semibold text-muted mb-1.5 block">الدور</label>
          <select name="role" x-model="form.role" class="w-full bg-canvas border border-line rounded-xl px-3 py-2.5 text-sm">
            @foreach($roleOptions as $k=>$v)<option value="{{ $k }}">{{ $v }}</option>@endforeach
          </select></div>
